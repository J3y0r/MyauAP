...
public class Clientinfo extends GuiScreen {
    ...
    private static final String CLIENT_WEBSITE  = "https://myau.wcgov.cn";
    private static final String CLIENT_WEBSITE2 = "https://os.mcsc.top";
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        mc.fontRendererObj.drawString(
                CLIENT_WEBSITE2,
                rightX - mc.fontRendererObj.getStringWidth(CLIENT_WEBSITE2),
    ...
    private void drawRoundedRect(float x1, float y1, float x2, float y2, float r, int color) {
        float[] c = rgba(color);
        setupGL(c);
        // 修复：quad() 各自管理自己的 begin/end，这里不能再套一层 glEnd
        quad(x1 + r, y1,     x2 - r, y2);
        quad(x1,     y1 + r, x1 + r, y2 - r);
        quad(x2 - r, y1 + r, x2,     y2 - r);
        fan(x1 + r, y1 + r, r, 180, 270, c); // 左上
        fan(x2 - r, y1 + r, r, 270, 360, c); // 右上
        fan(x2 - r, y2 - r, r,   0,  90, c); // 右下
        fan(x1 + r, y2 - r, r,  90, 180, c); // 左下
        cleanupGL();
    }
    ...
    private void drawRoundedOutline(float x1, float y1, float x2, float y2,
        ...
        GL11.glVertex2f(x1 + r, y1); GL11.glVertex2f(x2 - r, y1); // 上
        GL11.glVertex2f(x1 + r, y2); GL11.glVertex2f(x2 - r, y2); // 下
        GL11.glVertex2f(x1, y1 + r); GL11.glVertex2f(x1, y2 - r); // 左
        GL11.glVertex2f(x2, y1 + r); GL11.glVertex2f(x2, y2 - r); // 右
        GL11.glEnd();

        arc(x1 + r, y1 + r, r, 180, 270); // 左上
        arc(x2 - r, y1 + r, r, 270, 360); // 右上
        arc(x2 - r, y2 - r, r,   0,  90); // 右下
        arc(x1 + r, y2 - r, r,  90, 180); // 左下
