...
public class AutoL extends Module {
    ...
    private static final String[] POEMS = {
            ...
            "承让承让，阁下还是先练练基本功吧。",
            "以子之矛攻子之盾，子之矛已折矣。",
            "技之高低，不在一时胜负，而在尔已连败三局。",
            "汝当知，天外有天，人外有人。",
    };
    private static final String[] SAN_GUO = {
            "一破：卧龙出山，你已被MyauAP客户端击毙",
            "双连：一战成名，你已被MyauAP客户端击毙",
            "三连：举世皆惊，你已被MyauAP客户端击毙",
            "四连：天下无敌，你已被MyauAP客户端击毙",
            "五连：诛天灭地，你已被MyauAP客户端击毙"
    };
    ...
    private final ModeProperty wordPattern = new ModeProperty("pattern", 0,
            new String[]{"POEM", "MAMA", "PRIDEPLUS", "CIXIAOGUI", "CRYSTALPVP", "CLEAR", "ENGLISH", "SANGUO", "CUSTOM"});
    private final BooleanProperty nameInFront = new BooleanProperty("name-in-front", true);
    ...
    private String getRandomMessage() {
        ...
        switch (mode.toLowerCase()) {
            ...
            case "sanguo":
                return SAN_GUO[random.nextInt(SAN_GUO.length)];
            case "clear":
                return CLEAR[random.nextInt(CLEAR.length)];
            case "english":
                return ENGLISH[random.nextInt(ENGLISH.length)];
            case "mama":
                return MAMA[random.nextInt(MAMA.length)];
            case "prideplus":
                return PRIDEPLUS[random.nextInt(PRIDEPLUS.length)];
            case "cixiaogui":
                return CIXIAOGUI[random.nextInt(CIXIAOGUI.length)];
            case "crystalpvp":
                return CRYSTAL_PVP[random.nextInt(CRYSTAL_PVP.length)];
            case "poem":
            default:
                return POEMS[random.nextInt(POEMS.length)];
        }
    }
