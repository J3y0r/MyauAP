...
public class ChestAura extends Module {
    ...
    private TileTarget findTarget() {
        ...
        if (this.lockedTargetPos != null && now <= this.lockedTargetUntil) {
            ...
            TileTarget lockedTarget = lockedTile == null || this.shouldNotClickTile(lockedTile) ? null : this.findBestClickTarget(lockedTile, eyes, rangeSq, wallsRangeSq);
        ...
        if (this.target != null) {
            ...
            TileTarget currentTarget = currentTile == null || this.shouldNotClickTile(currentTile) ? null : this.findBestClickTarget(currentTile, eyes, rangeSq, wallsRangeSq);
        ...
        for (TileEntity tile : mc.theWorld.loadedTileEntityList) {
            if (this.shouldNotClickTile(tile)) {
                continue;
            }
    ...
    private TileTarget findBestClickTarget(TileEntity tile, Vec3 eyes, double rangeSq, double wallsRangeSq) {
        ...
        candidatePoints.sort(Comparator.comparingDouble(eyes::squareDistanceTo));
        ...
        for (Vec3 point : candidatePoints) {
            ...
            if (visible) {
                return new TileTarget(tile.getPos(), point, distanceSq, centerDistanceSq);
            }
            ...
            double tracedDistanceSq = trace.hitVec != null ? eyes.squareDistanceTo(trace.hitVec) : distanceSq;
    ...
    private boolean shouldNotClickTile(TileEntity tile) {
    ...
    private void tryOpenTarget(TileTarget openTarget, float yaw, float pitch) {
        ...
        if (tile == null || this.shouldNotClickTile(tile)) {
            return;
        }
    ...
    private void handleBlockAction(S24PacketBlockAction packet) {
        ...
        this.chestOpenMap.put(pos, new OpenRecord(now));
    ...
    private void markOpenedBlock(BlockPos pos) {
        ...
        for (EnumFacing facing : new EnumFacing[]{EnumFacing.NORTH, EnumFacing.SOUTH, EnumFacing.WEST, EnumFacing.EAST}) {
            ...
            if (adjacentTile instanceof TileEntityChest) {
                ...
                this.chestOpenMap.put(adjacentPos, new OpenRecord(now));
            }
        }
    }
    ...
    private void cleanupOpenMap() {
        long now = System.currentTimeMillis();
        this.chestOpenMap.entrySet().removeIf(entry -> now - entry.getValue().time > OPEN_TRACK_MS);
        if (this.lockedTargetPos != null && now > this.lockedTargetUntil) {
            this.lockedTargetPos = null;
            this.lockedTargetUntil = 0L;
        }
    ...
    private static class TileTarget {
        ...
        private TileTarget(BlockPos blockPos, Vec3 clickVec, double distanceSq, double centerDistanceSq) {
